...
router.post("/register", async (req, res) => {
  try {
    const { username, UserUniqueid,password, role } = req.body;

    // Check if user exists
    const user = await pool.query("SELECT * FROM users WHERE user_name = $1", [UserUniqueid]);
    ...
    const newUser = await pool.query(
      "INSERT INTO users (user_name, user_uid, user_password, user_role) VALUES ($1, $2, $3, $4) RETURNING *",
      [username, UserUniqueid,password, role]
    );
